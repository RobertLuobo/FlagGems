...
def greater_scalar_out(A, B, *, out=None):
    ...
    # scalarizes the tensor-vs-scalar compare (see the header comment); the
    # payload streams A once at the same memory footprint as ATen. Returns None
    # only when the payload does not apply (unsupported dtype/layout/out), in
    # which case the original paths below handle it.
    if A.numel() >= _RAW_SMALL_SCALAR_LIMIT:
