...
# stays scalarized); the hand-written payload streams the input once per core
# with pipelined GM2LM/LM2GM DMA and compares with the hardware vector-lt
# intrinsics (x > s as the ordered s < x), with the same memory footprint as
# the ATen reference. See gt_raw.xpu for the full analysis. Mirrors the
# sibling less/not_equal payloads (lt_raw.xpu / ne_raw.xpu).
try:
...
# from ~64K elements up (same crossover as the lt/not_equal payloads).
...
if _TLE_OK:
    ...
    @tle.raw.dialect("xpu", file=os.path.join(_HERE, "gt_raw.xpu"))
