...
def greater_scalar(A, B):
    ...
    # Fast path first: large contiguous float tensors, tile-divisible numel,
    # enough grid, scalar exactly representable in A.dtype.
    if (
        A.is_contiguous()
    ...
    # Fallback: enable the XPU compare-fusion pass for the generic scalar
    # compile. Only TRITONXPU_COMPARE_FUSION, never TRITONXPU_FP16_FAST; the
    # 16-bit branch of greater_func_scalar compares in the tensor's own dtype,
    # which is what makes the fused compare type-consistent (see the header
    # comment above).
...
def greater_scalar_out(A, B, *, out=None):
    ...
    # See greater_scalar: same fast-path gate (big contiguous float tensors,
