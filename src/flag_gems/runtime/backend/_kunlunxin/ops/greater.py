...
def greater_scalar_out(A, B, *, out=None):
    ...
    if A.numel() >= _RAW_SMALL_SCALAR_LIMIT:
        ...
        if raw_out is not None:
            return raw_out
    # Two-stage saturating fast path: big contiguous float tensors with a
    # tile-divisible numel, enough grid and a scalar exactly representable in
    ...
    # greater_func_scalar(out0=out) path, behavior unchanged. See
    # greater_scalar: no fusion env vars on the scalar path (fp16 compile).
    if (
        out is not None
        and A.is_contiguous()
        and out.is_contiguous()
    ...
    if A.numel() < _SMALL_SCALAR_LIMIT:
        # Small shapes: single launch, no intermediate tensor. COMPARE_FUSION
        # (set by _scalar_fusion_env) fuses the compare into the vectorized
        # load/store pipeline (~7.8x on fp16 with a fresh cache); the 16-bit
        # native-dtype compare in greater_func_scalar keeps the fused compare
        # same-typed (no fp16->fp32 promotion), hence FP16_FAST must NOT be set
        # together with it.
        prev = _scalar_fusion_env()
        try:
            if out is None:
