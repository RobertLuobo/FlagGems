...
def _is_all_true(inp):
    ...
    # Flat fallback: small, non-contiguous, or no clean power-of-2 column
    # width (the 2D path above did not return). Input dtype is asserted bool
    # above, so no `!= 0` widening is needed beyond what the kernel already
    # does.
