...
def lt_scalar(A, B):
    ...
    if A.numel() >= _RAW_SMALL_SCALAR_LIMIT:
        ...
        if raw_out is not None:
            return raw_out
    # Unmasked flat-tile Triton fast path (defined below): large contiguous
    # float tensors whose numel is an exact multiple of _LT_SCALAR_FAST_TILE
    # with scalar == 0.0. Kept as a fallback when the raw payload does not
    # apply (e.g. TLE unavailable / non-contiguous / odd shapes).
    numel = A.numel()
    if (
        A.is_contiguous()
