...
#
# Compared with the generic src/flag_gems/ops/linear.py kernel, this variant
# specializes the hot k-loop loads: when M % BLOCK_M == 0, N % BLOCK_N == 0 and
# K % BLOCK_K == 0 (all benchmark/real-world GEMM shapes), every load/store
# mask is statically all-true, so the masked-memory path (which on triton_xpu
# pays a per-lane predication/select cost and prevents block-DMA lowering) is
# replaced by unmasked block loads.  EVEN is a tl.constexpr so each
# specialization compiles only one load path (a runtime-scalar branch would
# materialise both paths and overflow uni_sram on this backend); the same
# autotune config set and the same tiling/accumulation semantics are kept as
# the generic kernel (fp32 accumulate, allow_tf32=False).
import logging
