...
@triton.jit
def _ctz(x): 
    return (libdevice.ffs(x) - 1).to(x.dtype)
...
def _binary_gcd(ax, ay, normal):
    ...
    one = (ax * 0 + 1)  # 与 ax 同类型的常量 1
    common = _ctz(tl.where(both_nonzero, ax | ay, one))
    u = tl.where(both_nonzero, ax >> _ctz(tl.where(both_nonzero, ax, one)), ax)
    v = ay
    active = both_nonzero

    while tl.sum(active.to(tl.int32), axis=0) > 0:
        one_v = (v * 0 + 1)  # 与 v 同类型的常量 1
        v_shifted = tl.where(active, v >> _ctz(tl.where(active, v, one_v)), v)
...
def gcd_kernel_i32(x_ptr, y_ptr, out_ptr, n_elements, BLOCK: tl.constexpr):
    ...
    while tl.sum(special.to(tl.int32), axis=0) > 0: 
        one_i32 = tl.full(sa.shape, 1, dtype=tl.int32)
        next_sa = tl.where(special, _c_rem_i32(sb, tl.where(special, sa, one_i32)), sa)
...
def gcd_kernel_i64(x_ptr, y_ptr, out_ptr, n_elements, BLOCK: tl.constexpr):
    ...
    special = special_mask & (sa != 0)
    while tl.sum(special.to(tl.int32), axis=0) > 0:
        one_i64 = tl.full(sa.shape, 1, dtype=tl.int64)
        next_sa = tl.where(special, _c_rem_i64(sb, tl.where(special, sa, one_i64)), sa)
        sb = tl.where(special, sa, sb)
        sa = next_sa
        special = special & (sa != 0)
