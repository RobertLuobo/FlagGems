...
def _tril_tile_kernel(
    ...
    # 用 row tensor 推进指针，使 ptr 变为 tensor pointer
    offs_m = pid_m * BLOCK_M + tl.arange(0, BLOCK_M)[:, None]
    offs_n = pid_n * BLOCK_N + tl.arange(0, BLOCK_N)[None, :]
    mask = (offs_m < M) & (offs_n < N)

    # 指针通过 tensor offset 推进（类似 triu.py 的 X += row * N）
    ...
    if col_start > row_end + diag:
        tl.store(out_ptr + offs_n, 0.0, mask=mask)
        return

    if col_end <= row_start + diag:
        x = tl.load(in_ptr + offs_n, mask=mask, other=0.0)
        tl.store(out_ptr + offs_n, x, mask=mask)
        return

    keep = offs_n <= (offs_m + diag)
    x = tl.load(in_ptr + offs_n, mask=mask & keep, other=0.0)
    tl.store(out_ptr + offs_n, x, mask=mask)
