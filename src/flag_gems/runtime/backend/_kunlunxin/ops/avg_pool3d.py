...
def avg_pool3d_forward_kernel(
    ...
    h_out_offsets = (h_block_idx * BLOCK_H + tl.arange(0, BLOCK_H)).to(tl.int64)
    w_out_offsets = (w_block_idx * BLOCK_W + tl.arange(0, BLOCK_W)).to(tl.int64)
    ...
    for kd in range(0, kernel_d):
        d_in = (d_idx * stride_d - padding_d + kd * dilation_d).to(tl.int64)
        d_valid = (d_in >= 0) & (d_in < in_d)
        for kh in range(0, kernel_h):
            for kw in range(0, kernel_w):
                h_in = (h_out_offsets[:, None] * stride_h - padding_h + kh * dilation_h).to(tl.int64)
                w_in = (w_out_offsets[None, :] * stride_w - padding_w + kw * dilation_w).to(tl.int64)
    ...
    if divisor_override != 0:
        ...
        d_start_fwd = (d_idx * stride_d - padding_d).to(tl.int64)
        ...
        h_start_fwd = (h_out_offsets[:, None] * stride_h - padding_h).to(tl.int64)
        ...
        w_start_fwd = (w_out_offsets[None, :] * stride_w - padding_w).to(tl.int64)
    ...
    out_h_offsets = (h_block_idx * BLOCK_H + tl.arange(0, BLOCK_H)).to(tl.int64)
    out_w_offsets = (w_block_idx * BLOCK_W + tl.arange(0, BLOCK_W)).to(tl.int64)
    output_block_ptr = (
