...
def test_empty(shape, dtype):
    # empty() returns uninitialized memory, so the element values are not part
    # of the contract and cannot be compared against a reference (the
    # allocation engine is free to return any bytes). What must hold is the
    # allocation contract: same shape/strides/dtype/numel/device and a
    # non-overlapping, dense, writable buffer. The reference is built the same
    # way to keep the comparison about the allocation contract only.
    ...
    assert res_out.numel() == ref_out.numel()
    assert res_out.device == ref_out.device
    # A fresh contiguous allocation stays non-overlapping, dense and writable.
    assert torch.ops.aten.is_non_overlapping_and_dense(res_out)
    res_out.zero_()
    assert torch.count_nonzero(res_out) == 0


@pytest.mark.empty
def test_empty_default_dtype():
    # Tests empty() with default dtype (not explicitly specified) to verify
    # proper dtype inference when only shape and device are given. Values are
    # uninitialized; only the allocation contract is checked.
    with flag_gems.use_gems():
        res_out = torch.empty(10, 20, device=flag_gems.device)

    assert res_out.shape == (10, 20)
    assert res_out.stride() == (20, 1)
    assert res_out.dtype == torch.get_default_dtype()
    assert res_out.numel() == 200
